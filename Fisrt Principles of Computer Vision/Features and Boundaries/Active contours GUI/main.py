...
def Blur_sliders(img: np.array) -> np.array:
    ...
    def update(val):
        ...
        blurred_img = cv2.GaussianBlur(
            img, (size.val, size.val), sigmaX=sigma.val)
...
def Blurred_Gradient_Magnitude_Squared(img: np.array) -> np.array:
    ...
    def update(val):
        ...
        BGMS_ = cv2.GaussianBlur(
            Squared_Mag, (size.val, size.val), sigmaX=sigma.val)
...
def get_Energy(BGMS: np.array, contour: np.array, size: int, bgr_img: np.array) -> None:
    ...
    for i in range(5):
        for v in range(points):
            ...
            if v + 1 < points:
                E_v_elastic = (contour[1][v + 1] - contour[1][v]
                               )**2 + (contour[0][v + 1] - contour[1][v])**2
                E_v_smooth = (contour[1][v + 1] - 2*contour[1][v] + contour[1][v - 1])**2 + (
                    contour[0][v + 1] - 2*contour[0][v] + contour[0][v - 1])**2
            else:
                E_v_elastic = (contour[1][0] - contour[1][v]
                               )**2 + (contour[0][0] - contour[1][v])**2
                E_v_smooth = (contour[1][0] - 2*contour[1][v] + contour[1][v - 1])**2 + (
                    contour[0][0] - 2*contour[0][v] + contour[0][v - 1])**2
